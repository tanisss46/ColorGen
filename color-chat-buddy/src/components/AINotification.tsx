...
export const AINotification = ({ onClose }: AINotificationProps) => {
  ...
  return (
    <div className="fixed top-[100px] left-[260px] z-50 fadeIn animate-fadeIn">
      <div className="p-2 bg-gradient-to-r from-blue-200/95 to-pink-200/95 rounded-lg shadow-md border border-purple-200 max-w-[300px] relative">
        <button 
          onClick={handleClose}
          className="absolute top-2 right-2 p-2.5 text-gray-600 hover:text-gray-800 rounded-full hover:bg-gray-200/50 transition-colors"
          aria-label="Close notification"
          style={{ minWidth: '30px', minHeight: '30px', touchAction: 'manipulation' }}
        >
          <X size={18} className="relative" />
        </button>
        
        <div className="flex items-start gap-2 pb-1 pr-6">
